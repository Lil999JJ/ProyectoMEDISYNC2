...
Generado el: {datetime.now().strftime('%d/%m/%Y %H:%M')}
    ...
    def create_integrated_billing_content(self, parent):
        """Crear contenido principal de facturación"""
        # Panel superior: Selección de cita
        citas_frame = tk.LabelFrame(parent, text="📅 SELECCIONAR CITA PARA FACTURAR", 
                                   font=('Arial', 12, 'bold'), bg='#e3f2fd', fg='#1565c0', 
                                   padx=15, pady=10)
        citas_frame.pack(fill='x', padx=10, pady=(10, 5))
        
        # Filtros de citas
        filters_frame = tk.Frame(citas_frame, bg='#e3f2fd')
        filters_frame.pack(fill='x', pady=(0, 10))
        
        tk.Label(filters_frame, text="Filtrar por:", bg='#e3f2fd', fg='#1565c0', font=('Arial', 10, 'bold')).pack(side='left')
        
        self.appointment_filter_var = tk.StringVar(value="completadas")
        filter_combo = ttk.Combobox(filters_frame, textvariable=self.appointment_filter_var,
        ...
        tk.Button(filters_frame, text="🔄 Actualizar", command=self.load_integrated_billing_data,
                 bg='#2196f3', fg='white', font=('Arial', 9, 'bold')).pack(side='left', padx=(10, 0))
        
        # Tabla de citas
        columns_citas = ('ID', 'Fecha', 'Paciente', 'Doctor', 'Estado', 'Facturada')
        self.appointments_tree_billing = ttk.Treeview(citas_frame, columns=columns_citas, 
                                                     show='headings', height=8)
        
        for col in columns_citas:
            self.appointments_tree_billing.heading(col, text=col)
            self.appointments_tree_billing.column(col, width=100, anchor='center')
        
        scroll_citas = ttk.Scrollbar(citas_frame, orient="vertical", command=self.appointments_tree_billing.yview)
        self.appointments_tree_billing.configure(yscrollcommand=scroll_citas.set)
        
        table_frame_citas = tk.Frame(citas_frame, bg='#e3f2fd')
        table_frame_citas.pack(fill='both', expand=True)
        
        self.appointments_tree_billing.pack(side='left', fill='both', expand=True)
        ...
        billing_main_frame = tk.Frame(parent, bg='#F8FAFC')
        billing_main_frame.pack(fill='both', expand=True, padx=10, pady=5)
        
        # Panel izquierdo: Servicios disponibles
        left_panel = tk.Frame(billing_main_frame, bg='#F8FAFC', width=400)
        left_panel.pack(side='left', fill='y', padx=(0, 5))
        left_panel.pack_propagate(False)
        
        services_frame = tk.LabelFrame(left_panel, text="🏥 SERVICIOS MÉDICOS", 
                                      font=('Arial', 11, 'bold'), bg='#e8f5e8', fg='#2e7d32',
                                      padx=10, pady=10)
        services_frame.pack(fill='both', expand=True)
        
        # Búsqueda de servicios
        search_frame = tk.Frame(services_frame, bg='#e8f5e8')
        search_frame.pack(fill='x', pady=(0, 10))
        
        tk.Label(search_frame, text="Buscar:", bg='#e8f5e8', fg='#2e7d32', font=('Arial', 10, 'bold')).pack(side='left')
        self.service_search_var = tk.StringVar()
        search_entry = tk.Entry(search_frame, textvariable=self.service_search_var, width=20)
        search_entry.pack(side='left', padx=(5, 0), fill='x', expand=True)
        search_entry.bind('<KeyRelease>', self.filter_services)
        
        # Tabla de servicios con scrollbars mejoradas
        columns_services = ('Código', 'Servicio', 'Categoría', 'Precio')
        
        # Frame contenedor para tabla y scrollbars
        table_frame_services = tk.Frame(services_frame, bg='#e8f5e8')
        table_frame_services.pack(fill='both', expand=True)
        
        self.services_tree_billing = ttk.Treeview(table_frame_services, columns=columns_services, 
                                                 show='headings', height=12)
        
        widths_services = {'Código': 80, 'Servicio': 150, 'Categoría': 100, 'Precio': 80}
        for col in columns_services:
            self.services_tree_billing.heading(col, text=col)
            self.services_tree_billing.column(col, width=widths_services.get(col, 100), anchor='center')
        
        # Scrollbars verticales y horizontales con mejor visibilidad
        scroll_services_y = ttk.Scrollbar(table_frame_services, orient="vertical", command=self.services_tree_billing.yview)
        scroll_services_x = ttk.Scrollbar(table_frame_services, orient="horizontal", command=self.services_tree_billing.xview)
        self.services_tree_billing.configure(yscrollcommand=scroll_services_y.set, xscrollcommand=scroll_services_x.set)
        
        # Layout con grid para mejor control de scrollbars con padding
        self.services_tree_billing.grid(row=0, column=0, sticky='nsew', padx=(5, 0), pady=(5, 0))
        scroll_services_y.grid(row=0, column=1, sticky='ns', padx=(0, 5), pady=(5, 0))
        scroll_services_x.grid(row=1, column=0, sticky='ew', padx=(5, 0), pady=(0, 5))
        
        # Configurar expansión con mínimo para scrollbars
        table_frame_services.grid_rowconfigure(0, weight=1)
        table_frame_services.grid_rowconfigure(1, weight=0, minsize=20)
        table_frame_services.grid_columnconfigure(0, weight=1)
        table_frame_services.grid_columnconfigure(1, weight=0, minsize=20)
        
        # Botón agregar servicio
        tk.Button(services_frame, text="➕ Agregar Servicio", command=self.add_service_to_invoice,
                 bg='#4caf50', fg='white', font=('Arial', 10, 'bold')).pack(pady=(10, 0))
        
        # Panel derecho: Factura en proceso
        right_panel = tk.Frame(billing_main_frame, bg='#F8FAFC')
        right_panel.pack(side='right', fill='both', expand=True, padx=(5, 0))
        
        invoice_frame = tk.LabelFrame(right_panel, text="📄 FACTURA EN PROCESO", 
                                     font=('Arial', 11, 'bold'), bg='#fff3e0', fg='#e65100',
                                     padx=10, pady=10)
        invoice_frame.pack(fill='both', expand=True)
        
        # Información del paciente seleccionado
        patient_info_frame = tk.Frame(invoice_frame, bg='#fff3e0', relief='solid', bd=1)
        patient_info_frame.pack(fill='x', pady=(0, 10))
        
        self.patient_info_label = tk.Label(patient_info_frame, text="Seleccione una cita para continuar",
                                          bg='#fff3e0', fg='#e65100', font=('Arial', 10, 'bold'))
        self.patient_info_label.pack(pady=10)
        
        # Tabla de servicios en factura con scrollbars mejoradas
        columns_invoice = ('Servicio', 'Cantidad', 'Precio Unit.', 'Total')
        
        # Frame contenedor para tabla y scrollbars
        table_frame_invoice = tk.Frame(invoice_frame, bg='#fff3e0')
        table_frame_invoice.pack(fill='both', expand=True, pady=(10, 0))
        
        self.invoice_services_tree = ttk.Treeview(table_frame_invoice, columns=columns_invoice, 
                                                 show='headings', height=8)
        
        widths_invoice = {'Servicio': 150, 'Cantidad': 80, 'Precio Unit.': 100, 'Total': 100}
        for col in columns_invoice:
            self.invoice_services_tree.heading(col, text=col)
            self.invoice_services_tree.column(col, width=widths_invoice.get(col, 100), anchor='center')
        
        # Scrollbars verticales y horizontales con mejor visibilidad
        scroll_invoice_y = ttk.Scrollbar(table_frame_invoice, orient="vertical", command=self.invoice_services_tree.yview)
        scroll_invoice_x = ttk.Scrollbar(table_frame_invoice, orient="horizontal", command=self.invoice_services_tree.xview)
        self.invoice_services_tree.configure(yscrollcommand=scroll_invoice_y.set, xscrollcommand=scroll_invoice_x.set)
        
        # Layout con grid para mejor control de scrollbars con padding
        self.invoice_services_tree.grid(row=0, column=0, sticky='nsew', padx=(5, 0), pady=(5, 0))
        scroll_invoice_y.grid(row=0, column=1, sticky='ns', padx=(0, 5), pady=(5, 0))
        scroll_invoice_x.grid(row=1, column=0, sticky='ew', padx=(5, 0), pady=(0, 5))
        
        # Configurar expansión con mínimo para scrollbars
        table_frame_invoice.grid_rowconfigure(0, weight=1)
        table_frame_invoice.grid_rowconfigure(1, weight=0, minsize=20)
        table_frame_invoice.grid_columnconfigure(0, weight=1)
        table_frame_invoice.grid_columnconfigure(1, weight=0, minsize=20)
        
        # Botones de servicio
        service_buttons_frame = tk.Frame(invoice_frame, bg='#fff3e0')
        service_buttons_frame.pack(fill='x', pady=(10, 0))
        
        tk.Button(service_buttons_frame, text="❌ Quitar", command=self.remove_service_from_invoice,
                 bg='#f44336', fg='white', font=('Arial', 9, 'bold')).pack(side='left', padx=(0, 5))
        tk.Button(service_buttons_frame, text="✏️ Editar", command=self.edit_service_in_invoice,
                 bg='#ff9800', fg='white', font=('Arial', 9, 'bold')).pack(side='left')
        
        # Panel de cálculos
        self.create_billing_calculations_panel(invoice_frame)
...
"""
    ...
    def create_advanced_billing_tab(self, parent):
        ...
        # Treeview para citas con altura aumentada
        columns = ('ID', 'Fecha', 'Paciente', 'Doctor', 'Motivo', 'Estado')
        self.billing_appointments_tree = ttk.Treeview(appointments_frame, columns=columns, show='headings', height=15)
        ...
        # Treeview para facturas con altura aumentada
        invoice_columns = ('Número', 'Fecha', 'Paciente', 'Monto', 'Estado', 'Acciones')
        self.billing_invoices_tree = ttk.Treeview(invoices_frame, columns=invoice_columns, show='headings', height=15)
